...
/* Calculates demand based on listings and calendar joins, and saves to Demand.location collection. */
...
/* Iterate over listings and for each calls the airbnb calendars endpoint. Saves to calendars collection. */
...
/* Save demand array to json file in the ./json folder. */
function saveToJson(demandArray, location) {
    if (!demandArray) return;
    const jsonFilename = 'Demand.' + _.chain(location).trim().startCase() + '.json';
    log(`Writing demand to JSON file json/${jsonFilename}...`);
    try {
        fs.writeFileSync(path.join(__dirname, 'json', jsonFilename), JSON.stringify(demandArray.map(r => {
            delete r._id;
            return r;
        })), 'utf-8');
    } catch (err) {
...
async function run(location, limit) {
    try {
        ...
        saveToJson(demandArr, location);
        mongoose.connection.close();
        log('get-airbnb-demand: DONE.');
    } catch (err) {
        error('Error', err);
    }
}
